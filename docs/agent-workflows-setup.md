...
Run `node --import tsx --test test/market-research.test.ts test/turn-presentation.test.ts` for the focused mocked contract, arithmetic, intake, auth and UI-progress checks. Test facts use the provider's published response shape with synthetic Portuguese properties; they are not live listing data. The public contract was inspected on 8 September 2026. Live subscription/Portugal response verification remains pending until `RAPIDAPI_KEY` is supplied. `get_workflow_status` distinguishes configured credentials from an actual connectivity check.
