...
Status: implemented locally, 8 September 2026. Live Portugal autocomplete and search verified on 9 September; the parser now accepts the live `query`/`listings` response as well as the published example shape. Two returned Carcavelos Centro listings were retained; the three-comparable minimum correctly withholds an estimate and returns both links. See [setup and verification](agent-workflows-setup.md#idealista-market-research).

Initial validation (8 September): 17 focused checks passed; backend TypeScript and production web builds passed. Full suite: 226 passed, one skipped, one existing failure in `gateway-media.test.ts` (the brochure send-failure test expects a rejection that the gateway handles). Two pre-existing TypeScript inference errors in property-title fallback and attachment cleanup were corrected without changing runtime behaviour. Browser checks used the real chat component with synthetic API/message fixtures: starter submission, research progress, comparison table/links and history reload passed. The authenticated live chat and live Idealista results were not exercised in that initial check. No new dependency or migration was added.
...
Implementation inspected the OpenAPI document embedded in the supplied RapidAPI page. The selected endpoint is `/auto-complete`; its IDs feed `/property-search`. The contract defines `min_size`/`max_size` as built area. Although it calls `min_rooms` a minimum bedroom filter, live Portugal verification on 9 September showed exact T0–T3 buckets and a T4+ bucket. Market research requests one bucket and checks exact bedrooms locally; buyer matching requests every applicable bucket through `filters.bedrooms`. The code accepts both the documented `actualPage` and live `currentPage` response fields. Live Portugal verification confirmed `query`/`listings` rather than the published examples' `filters`/`elementList`; both are supported. No query fields were borrowed from another provider.
