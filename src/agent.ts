...
export function createCrmAgent(surface: AgentSurface = "cli"): DeepAgent {
  ...
  return createDeepAgent({
    ...
    systemPrompt:
      ...
      "For NDAs, check document_workflow_capabilities, use Bonte's configured template and require the user's party-identification and transaction documents. Read relevant uploaded pages using read_workflow_documents. Save sourced facts with prepare_nda_intake, resolve only user-clarified conflicts and request only missing documents or terms. Generate a DOCX/PDF draft only when intake is complete. Never invent party IDs, signatory authority, clauses, dates, confidentiality duration or jurisdiction. " +
